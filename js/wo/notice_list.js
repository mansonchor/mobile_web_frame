/*
    hdw 消息列表
*/
define("wo/notice_list",["base_package",'frame_package',"btn_package","commom_function","wo_config","get_template","world_list_module","message_nav","user_list_view","user_list_controler","user_list_collection","notice","system_notice_view","no_login"],function(require, exports)
{
	var $ = require('zepto')
	...
	exports.route = { "notice_list": "notice_list" }

	exports.new_page_entity = function()
	{
		var options = {		
			title : "系统通知",
			route : { "notice_list": "notice_list" },
		...
		var user_list_collection_obj = new_user_list_collection({
			...
			refresh : function()
			{
				collection_options.data = { get_read_status : "unread" ,notice_group : "normal_group" } 
			...
			get_more_item : function()
			{
				...
				collection_options.data = { get_read_status : "all"  ,notice_group : "normal_group"}
		...
		options.window_change = function(page_view)
		{
            ...
            _page_view.$el.find(".no_data").height(common_function.container_height_with_head_and_nav())
		}
		...
		{   
		    ...
		    window.localStorage.setItem("nav_address","notice_list");
            ...
            else
            {
				...
				else
				{
                    ...
                    if(notice.get_unread_count_by_type("system")>0)
                    {
                        user_list_collection_obj.refresh()    
                    }
				}
